...
| `risk.min_order_btc` | 最小注文数量（bitbank の BTC/JPY 板取引は 0.0001 BTC） |
...
1 回の注文額の下限は `min_order_jpy` と、`min_order_btc` を円に直した額の大きいほう。
BTC が 1,200 万円なら 0.0001 BTC = 約 1,200 円なので、資金 1 万円でも 8 分割まではできる。
最小単位が効いてくるのは、分割数を増やしたときか BTC が大きく上がったときだけ
（`order_ratio: 0.3` なら BTC が 3,300 万円を超えたあたり）。

なお販売所なら 0.00000001 BTC から買えるが、ccxt が持つのは板取引の
エンドポイントだけなので、このボットからは使えない。スプレッドの差を考えても
板取引のほうが有利。
...
これを鵜呑みにすると「注文しただけ」を「約定した」として帳簿に書き、
実際の持ち高と静かにずれていく。`LiveBroker` は約定が確定するまで注文を見にいき、
待っても埋まらなければ**取り消してから**、実際に約定したぶんだけを記録する。
1 枚も約定しなければ何も記録しない。

- 記録するのは常に**実際の約定価格**。シグナル時の価格では代用しない
- 約定したのに価格が取れないときは、推測せずに落とす（誤った単価は損益にずっと残る）
- 想定より高く約定して残高を超えても、記録は拒まない。取引所で本当に買えている以上、
  帳簿上だけ持っていないことにするほうが危ない
- bitbank は注文の応答に手数料を含めないので、taker 手数料から見積もる。
  帳簿の手数料は概算で、取引所の請求額とは完全には一致しない
