...
**既定値は資金 1 万円を前提にしている。** 資金を変えるときは 4 つを合わせて動かすこと。

| キー | 1 万円（既定） | 10 万円 | 100 万円 |
| --- | --- | --- | --- |
| `paper.initial_jpy` | 10000 | 100000 | 1000000 |
| `risk.order_jpy` | 3000 | 10000 | 30000 |
| `risk.daily_loss_limit_jpy` | 1000 | 10000 | 100000 |
| `risk.max_position_btc` | 0.001 | 0.01 | 0.1 |

目安は「1 回の注文＝資金の 10〜30%」「当日の損失上限＝資金の 10%」。

1 万円だと 1 回の注文が資金の 3 割になる。これは荒いが、これ以上小さくすると
最小注文数量 0.0001 BTC を割りやすくなるための妥協で、少額運用の構造的な制約といえる。
`order_jpy` を下げるほど、最小単位を割る BTC 価格も下がってくる
（`order_jpy: 3000` なら BTC が 3,000 万円を超えた時点で発注できなくなる）。
`python -m bot.doctor` がその上限価格を表示するので、変更したら必ず確認すること。

手数料は率なので資金が小さくても不利にはならないが、利益の絶対額は小さくなる。
1 回 3,000 円の注文で相場が 2% 動いても、手取りは 50 円ほど。
