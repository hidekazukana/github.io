...
| キー | 既定 | 意味 |
| --- | --- | --- |
| `risk.order_ratio` | **1.0** | 1 回の買いに使う残高の割合。**1.0 = 全額ベット** |
| `risk.order_jpy` | `null` | 1 回の注文額の絶対上限。`null` なら上限なし |

割合で持っているので、残高が増えても減っても常に「その時点の全額」を張る
（`order_jpy` に金額を入れると、そこで頭を押さえられる）。分散させたいなら
`order_ratio: 0.3` のように下げる。

**全額ベットでは 1 回の判断に資金の 100% が乗る。** 上振れも下振れもそのまま資金に響く。
`python -m bot.doctor` は全額ベットの設定を `[注意]` として表示する。

全額ベットのとき、次の 2 つが「全額」を邪魔しないか確認すること。doctor がどちらも
効き始める価格を教えてくれる。

- `max_position_btc` … 資金で買える量より小さいと、全額を張る前に頭打ちになる
  （既定の 0.01 BTC は、資金 1 万円なら BTC が 100 万円まで下げても届かない量）
- `daily_loss_limit_jpy` … 全額ベットは 1 回の負けが大きいので、この上限に当たって
  その日は買いを止める、という動きが増える。それが狙いなら問題ないが、
  常に張り続けたいなら上げること

### 資金を変えるとき

`order_ratio` は割合なので触らなくてよい。資金額に紐づく 3 つだけ合わせる。

| キー | 1 万円（既定） | 10 万円 | 100 万円 |
| --- | --- | --- | --- |
| `paper.initial_jpy` | 10000 | 100000 | 1000000 |
| `risk.daily_loss_limit_jpy` | 1000 | 10000 | 100000 |
| `risk.max_position_btc` | 0.01 | 0.1 | 1.0 |

手数料は率なので資金が小さくても不利にはならないが、利益の絶対額は小さくなる。
資金 1 万円の全額ベットで相場が 2% 動いても、手取りは 170 円ほど。
